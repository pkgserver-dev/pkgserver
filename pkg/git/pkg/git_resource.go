...
func (r *gitRepository) getResources(ctx context.Context, pkgRev *pkgv1alpha1.PackageRevision, useWorkspaceBranch bool) (map[string]string, error) {
	...
	pkgRootTree, err := getPackageTree(ctx, pkgRev.Spec.PackageRevID, commit)
	if err != nil {
...
func (r *gitRepository) getCommit(ctx context.Context, pkgRev *pkgv1alpha1.PackageRevision, useWorkspaceBranch bool) (*object.Commit, error) {
	log := log.FromContext(ctx)
	log.Info("getCommit", "repo", r.cr.Name, "pkgID", pkgRev.Spec.PackageRevID)
	...
	if !useWorkspaceBranch && pkgRev.Spec.PackageRevID.Revision != "" {
		tagRefName := packageTagRefName(pkgRev.Spec.PackageRevID, pkgRev.Spec.PackageRevID.Revision)
		...
		_, commit, err := r.getBranchAndCommitFromTagHash(ctx, pkgRev.Spec.PackageRevID.Package, tagRef.Hash(), tagRefName.String())
		return commit, err
	} else {
...
func getPackageTree(ctx context.Context, pkgRevID pkgrevid.PackageRevID, commit *object.Commit) (*object.Tree, error) {
	log := log.FromContext(ctx)
	...
	tree, err := rootTree.Tree(pkgRevID.Path())
	if err != nil {
		if err == object.ErrDirectoryNotFound {
			// We treat the filter prefix as a filter, the path doesn't have to exist
			log.Info("could not find prefix in commit; returning no resources", "package", pkgRevID.Path(), "commit", commit.Hash.String())
			return nil, nil
		} else {
			return nil, fmt.Errorf("error getting tree %s: %w", pkgRevID.Path(), err)
		}
	}
	return tree, nil
}
