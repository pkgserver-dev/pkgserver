...
func (r *watcher) listAndWatch(ctx context.Context, l rest.RESTListStrategy, options *metainternalversion.ListOptions) {
	...
	if err := r.innerListAndWatch(ctx, l, options); err != nil {
		...
		ev := watch.Event{
			Type:   watch.Error,
			Object: &pkgv1alpha1.PackageRevisionResources{},
		}
		r.resultChan <- ev
	}
	r.cancel()
...
func (r *watcher) innerListAndWatch(ctx context.Context, l rest.RESTListStrategy, options *metainternalversion.ListOptions) error {
	...
	if !options.Watch {
		...
		pkgRevResourcesList, ok := obj.(*pkgv1alpha1.PackageRevisionResourcesList)
		if !ok {
			r.setDone()
			return fmt.Errorf("expecting type, got: %s", reflect.TypeOf(obj).Name())
		}
		for _, obj := range pkgRevResourcesList.Items {
			obj := obj
