/*
Copyright 2024 Nokia.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/
// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "github.com/pkgserver-dev/pkgserver/apis/config/v1alpha1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakePackageVariants implements PackageVariantInterface
type FakePackageVariants struct {
	Fake *FakeConfigV1alpha1
	ns   string
}

var packagevariantsResource = v1alpha1.SchemeGroupVersion.WithResource("packagevariants")

var packagevariantsKind = v1alpha1.SchemeGroupVersion.WithKind("PackageVariant")

// Get takes name of the packageVariant, and returns the corresponding packageVariant object, and an error if there is any.
func (c *FakePackageVariants) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.PackageVariant, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(packagevariantsResource, c.ns, name), &v1alpha1.PackageVariant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.PackageVariant), err
}

// List takes label and field selectors, and returns the list of PackageVariants that match those selectors.
func (c *FakePackageVariants) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.PackageVariantList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(packagevariantsResource, packagevariantsKind, c.ns, opts), &v1alpha1.PackageVariantList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.PackageVariantList{ListMeta: obj.(*v1alpha1.PackageVariantList).ListMeta}
	for _, item := range obj.(*v1alpha1.PackageVariantList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested packageVariants.
func (c *FakePackageVariants) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(packagevariantsResource, c.ns, opts))

}

// Create takes the representation of a packageVariant and creates it.  Returns the server's representation of the packageVariant, and an error, if there is any.
func (c *FakePackageVariants) Create(ctx context.Context, packageVariant *v1alpha1.PackageVariant, opts v1.CreateOptions) (result *v1alpha1.PackageVariant, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(packagevariantsResource, c.ns, packageVariant), &v1alpha1.PackageVariant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.PackageVariant), err
}

// Update takes the representation of a packageVariant and updates it. Returns the server's representation of the packageVariant, and an error, if there is any.
func (c *FakePackageVariants) Update(ctx context.Context, packageVariant *v1alpha1.PackageVariant, opts v1.UpdateOptions) (result *v1alpha1.PackageVariant, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(packagevariantsResource, c.ns, packageVariant), &v1alpha1.PackageVariant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.PackageVariant), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakePackageVariants) UpdateStatus(ctx context.Context, packageVariant *v1alpha1.PackageVariant, opts v1.UpdateOptions) (*v1alpha1.PackageVariant, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(packagevariantsResource, "status", c.ns, packageVariant), &v1alpha1.PackageVariant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.PackageVariant), err
}

// Delete takes name of the packageVariant and deletes it. Returns an error if one occurs.
func (c *FakePackageVariants) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteActionWithOptions(packagevariantsResource, c.ns, name, opts), &v1alpha1.PackageVariant{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakePackageVariants) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(packagevariantsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.PackageVariantList{})
	return err
}

// Patch applies the patch and returns the patched packageVariant.
func (c *FakePackageVariants) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.PackageVariant, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(packagevariantsResource, c.ns, name, pt, data, subresources...), &v1alpha1.PackageVariant{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.PackageVariant), err
}
